...
class AdminOrder extends React.Component {
    ...
    async componentDidMount(){
        ...
        let order = await adminService.handleGetOrderBySid(this.props.adminInfo.sid)
        this.setState({
            order: order
        })
    ...
    detailButton = async (orderId)=>{
        let data = await adminService.handleGetOrderItemBySidAndOrderId(orderId,this.props.adminInfo.sid)
        let tempOrder  = null
        ...
        return (
            <>
                                    ...
                                    return (
                                        ...
                                        <td>
                                            <div className="actions__status">
                                                ...
                                                :
                                                <>  
                                                    <button className="actions__status--accept" onClick={()=>this.changeOrderStatus(item.orderId,'Accepted')}>
                                                        Accept
                                                    </button>
                                                    <button className="actions__status--reject" onClick={()=>this.changeOrderStatus(item.orderId,'Rejected')}> 
                                                        Reject
                                                    </button>
                                                </>
                                                }
                                                <button className="actions__status--detail" onClick={()=>this.detailButton(item.orderId)} > 
                                                   Detail
                                                </button>
                                            </div>
                                        </td>
                ...
                <Modal isOpen={this.state.modal} toggle={this.toggle} className="order-detail-modal">
                    <OrderDetail data={this.state.detailData}  order={this.state.tmpOrder}/>
                </Modal>
            </>
        )
    }
